...
int main(){
  int radius;
  char chordVec[151];
  FILE *pacmanIncludeFile = fopen("abPacman_decls.h", "w");
  FILE *chordIncludeFile = fopen("chordVec.h", "w");
  assert(chordIncludeFile); assert(pacmanIncludeFile);

  fprintf(pacmanIncludeFile, "// Automatically generated by makePacmans.  (c) Eric Freudenthal, 2016\n");
  fprintf(pacmanIncludeFile, "#ifndef abPacman_decls_included\n#define abPacman_decls_included\n\n");
  ...
  for (radius = 2; radius <= 150; radius++) {
    ...
    {				/* chordVecN.c */
      sprintf(filename, "pacmans/chordVec%d.c", radius);
      FILE *fp = fopen(filename, "w");
      assert(fp);
      fprintf(fp, "// Automatically generated by makePacmans.  (c) Eric Freudenthal, 2016\n");
	...
	fprintf(fp, "    %d, // dist along axis = %d\n", chordVec[chordIndex], chordIndex);
      ...
      sprintf(filename, "pacmans/abPacman%d.c", radius);
      FILE *fp = fopen(filename, "w");
      assert(fp);
      fprintf(fp, "// Automatically generated by makePacmans.  (c) Eric Freudenthal, 2016\n");
      fprintf(fp, "#include \"abPacman.h\"\n\n");
      fprintf(fp, "#include \"chordVec.h\"\n\n");
      fprintf(fp, "const AbPacman pac%d = {" , radius);
      fprintf(fp, "  abPacmanGetBounds, abSlicedPacmanCheck, chordVec%d, %d", radius, radius);
      fprintf(fp, "};\n");
      fclose(fp);
    }
    				/* includes */
    fprintf(chordIncludeFile, "extern const unsigned char chordVec%d[%d];\n", radius, radius+1);
    fprintf(pacmanIncludeFile, "extern const AbPacman pac%d;\n" , radius);
  }

  fprintf(pacmanIncludeFile, "\n#endif // included \n");
  fprintf(chordIncludeFile, "\n#endif // included \n");
  fclose(chordIncludeFile);
  fclose(pacmanIncludeFile);
}
